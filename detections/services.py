"""Orchestration for the image workflow: scan, detect, classify, cluster.
...
from detections.models import Detection, Event, Image, SpeciesClassification
...
def ingest_directory(folder, batch_size=8, device="cpu", conf=None,
    ...
    discovered, created = _register_new_images(folder, limit)
...
def _register_new_images(folder, limit=None):
...
def read_captured_at(path):
    ...
    that captured_at was silently None for every image ever ingested — which in
    turn made event clustering produce nothing.
    """
    try:
...
def cluster_images(rebuild=False):
    """Group images into events by camera site and capture-time gap.

    Images at one camera_site start a new event whenever the gap to the previous
    capture exceeds EVENT_GAP_SECONDS. Images with no EXIF time cannot be placed
    on the timeline at all, so they are counted and skipped.
    """
    ...
    qs = Image.objects.filter(captured_at__isnull=False)
    if not rebuild:
    ...
    for img in qs.iterator():
        starts_new_group = (
            ...
            or previous is None
            or img.captured_at - previous.captured_at > gap
        )
        if starts_new_group:
            flush()
