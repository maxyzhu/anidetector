...
Runs the whole pipeline (ingest -> classify) against a throwaway test database
(auto-created, auto-destroyed) so it never touches your dev/prod data. Model load
times are measured separately from per-item throughput.
...
class Command(BaseCommand):
    ...
    def _profile(self, opts):
        ...
        # The test database is empty, so the run has to supply its own deployment.
        deployment = Deployment.objects.create(
            camera_id="profile", location="profile", country="USA",
        ...
        stage(
            "ingest",
            lambda: call_command("ingest", dataset, deployment=deployment.id,
                                 batch_size=batch_size, device=md_device,
                                 limit=limit, stdout=sink),
