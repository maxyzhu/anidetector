...
Lives in core because a track's representative frame needs the same drawing as a
still image, and video must not reach into the image pipeline to get it.
"""
...
def species_labels(detections):
    labels = []
    for detection in detections:
        row = list(detection.classifications.all())
        if not row:
            labels.append(f"{detection.category} {detection.confidence:.2f}")
            continue
        newest = row[0]
        taxon = parse_taxon_path(newest.category)
        name = (taxon.common_name if taxon else "") or newest.category
        confidence = newest.confidence
        labels.append(f"{name} {confidence:.2f}" if confidence else name)
    return labels


def annotate_image(image_path: str, detections, labels=None) -> bytes:
